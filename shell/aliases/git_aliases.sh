...
alias gl='gglg $(git show-ref | cut -d " " -f 2 | grep -v stash$) HEAD'
...
alias gbDm='git branch -vv | command grep "origin/.*: gone]" | command sed "s/\*//" | command awk '"'"'{print $1}'"'"' | command xargs -n 1 git branch -D'
...
alias gup='git pull --rebase --autostash && gfa'
